...
def analysis():
    if request.is_json:
        ...
        for speaker in request_array:
            textArray = speaker
            result = analyzeAllModel(corrected_texts = textArray, device = device, grammar_model = grammar_model, emotion_model = emotion_model, moral_model = moral_model, politely_model = politely_model,
                            grammar_tokenizer = grammar_tokenizer, emotion_tokenizer = emotion_tokenizer, moral_tokenizer = moral_tokenizer, politely_tokenizer = politely_tokenizer)
